        ...
        for epoch in range(1, self.EPOCHS):
                ...
                for idx, data in enumerate(norm_data_list):
                    try:
                        if nn.model_choice <= 4 or nn.model_choice == 11:
                            tmp_train.append(reshape(self.sampler.normalized_data[idx].to_numpy(), (126, 1)))
                        elif 5 <= nn.model_choice <= 6:
                            tmp_train.append(reshape(self.sampler.normalized_data[idx].to_numpy(), (130, 1)))
                        elif 7 <= nn.model_choice <= 11:  # 5 days * 11
                            tmp_train.append(reshape(self.sampler.normalized_data[idx].to_numpy(), (55, 1)))
            ...
            try:
                ...
                print("[ERROR] Failed to train dataset!\r\nInformation: ",
                      f"x size: {len(x)}",
                      f"x first val: {x[0].size}",
                      f"y size: {len(y)}",
                      f"y first val: {y[0].size}",
