        ...
        try:
            ...
            if not skip_db:
                ...
                fib_data = pd.DataFrame(
                    columns=['0.202', '0.236', '0.241', '0.273', '0.283', '0.316', '0.382', '0.5', '0.523', '0.618', '0.796',
            ...
            if len(date_range) == 0 and not self._force_generate and not skip_db:  # continue loop if found cached data
                ...
                tmp_new_set = new_set
                if new_set['Low'].idxmin() == 0 or new_set['Low'].idxmin() == len(new_set['Low']) -1:
                    tmp_new_set = new_set.drop(new_set['Low'].idxmin())
                new_set_dropped_min = tmp_new_set.drop(tmp_new_set['Low'].idxmin())
                while len(new_set_dropped_min) > 0 and new_set_dropped_min['Low'].idxmin() >= tmp_new_set['Low'].idxmin():
                    new_set_dropped_min = new_set_dropped_min.drop(new_set_dropped_min['Low'].idxmin())
                if len(new_set_dropped_min) != 0 and new_set_dropped_min['Low'].idxmin() <= len(tmp_new_set) - 4 and new_set['Low'].idxmin() <= len(tmp_new_set) - 4 and tmp_new_set['Low'].min() < data['Low'].iloc[-1]:
                    new_set = tmp_new_set
                    try:
