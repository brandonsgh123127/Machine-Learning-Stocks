...
class Gather():
    ...
    def gen_random_dates(self):
        ...
        self.date_set=self._reorder_dates(self.date_set[0],self.date_set[1])
        return self.date_set
    def get_data(self):
        return self.data
    # Twitter API Web Scraper for data on specific stocks
    def get_recent_news(self,term,start,end,amt=100):
        print(f'Retrieving news using Twitter API...') 
        results = self.api.GetSearch(
            raw_query=f'q=%24{term}%20min_faves%3A0%20&result_type=mixed%20&since={start}%20&until={end}%20&count={amt}'
            )
        return results