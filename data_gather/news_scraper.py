...
class News_Scraper(Gather):
    ...
    def _filter_terms(self,tweets):
        ...
        for tweet in tweets:
            if not pattern.match(tweet.text.lower()) and not self.filtered_results.hash.isin([hash(tweet.id)]).any().any() and len(tweet.text) < 150:
    ...
    def get_news(self,term,start_date):
        ...
        self._filter_terms(self.get_recent_news(term,start_date,amt=100))
